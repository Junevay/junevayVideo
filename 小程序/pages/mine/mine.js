...
Page({
  ...
  onLoad: function (params) {
    ...
    wx.request({
      url: serverUrl + '/user/getUserInfo?userId=' + userId + "&fanId=" + user.id,
      method: "POST",
      header: {
  ...
  logout: function () {
    ...
    //  
    wx.request({
      url: app.serverUrl + "/user/logout?userId=" + user.id,
      method: 'POST',
      header: {
        'content-type': 'application/json', // 默认值
        'headerUserId': user.id,
  ...
  getMyVideoList: function (page) {
    ...
    wx.request({
      url: serverUrl + '/video/showAll/?isSaveRecord=0&page=' + page + '&pageSize=6',
      method: "POST",
      data: {
